...
if(!empty($_POST)){ //Si le formulaire a été envoyé
    ...
    // Ici faudrait faire 9 conditions pour vérifier que les champs du formulaire sont bien remplis.

    $photo_bdd=''; // par défaut le champ sera vide en BDD

    //  - traitement de la photo :
    debug($_FILES); // La superglobale $_FILES a un indive "photo" qui cirrespond au "name" de l'input type="files" du formulaire, ainsi qu'un indice "name" qui contient le nom di fichier en cours de téléchargement

    if(!empty($_FILES['photo']['name'])){ // si le nom en cours de téléchargement n'est pas vide, alors c'est qu'on est en train de télécharger une photo
...
<form action="" method="post" enctype="multipart/form-data"><!-- l'attribut enctype spécifie que le formulaire envoie des fichiers en plus des données
    // 'text' permet de télécharger un fichier (photo) -->
...
<div>
    ...
    <div><input type="file" name="photo" id="photo">  </div> <!-- attention pour pouvoir utiliser le type="file" il ne faut pas oublier l'attribut enctype-->
