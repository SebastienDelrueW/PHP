...
if(!empty($_POST)) { // si le formulaire a été envoyé, $_POST n'est pas vide
    ...
    if(!isset($_POST['pseudo']) ||strlen($_POST['pseudo'])< 4 || strlen($_POST['pseudo']) > 20){ // si n'existe pas l'indice "pseudo" dans $_POST c'est que le formulaire a été modifié. 
        // Si la longueur du pseudo est inférieur à 4 pu supérieur à 20, on met un message d'erreur à l'internaut
    ...
    if(!isset($_POST['email']) || strlen($_POST['email']) > 50 || !filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)){ // LA fonction filter_var(retourne true si $_POST['email']) 
        //est bien     deformat email, sinon elle retourne false (ici on met un "!" pour not car on veut vérifier qu'il ne s'agit pas d'un email.)
    ...
    if(!isset($_POST['code_postal']) || !preg_match('#^[0-9]{5}$#',$_POST['code_postal'])){
        $contenu .='<div class="alert alert-danger"> Le Code Postal n\'est valide.</div>';
    }
    if(!isset($_POST['adresse']) ||strlen($_POST['adresse'])< 4 || strlen($_POST['adresse']) > 20){ 
    $contenu .='<div class="alert alert-danger"> L\'adresse doit contenir entre 4 et 20 caractères.</div>';
    }// preg_mach vérifie si le code postal correspond à l'expression régulière précisée.
    /*  Le regex s'écrit entre #
        Le ^  définit le début de l'expression
        Le $  définit la fin de l'expression
        [0-9] définit l'intervale des chiffres autorisés
        {5}   définit que l'on veut 5 précisément
    */

    // S'il n'y a pas d'erreur sur le formulaire, on vérifie que le pseudo est dispinible puis on insère le membre en bdd :
    if(empty($contenu)) { // si la variable est vide, c'est qu'il n'y a pas d'erreur sur le formulaire.

        // On sélectionne le pseudo on BDD :
        ...
        if($membre->rowCount() >0){ // Si la requête retourne des lignes c'est que le pseudo existe déjà.
            $contenu .='<div class="alert alert-danger"> Le pseudo est indisponible. Veuillez choisir un autre.</div>';
        } else { // On inscrit le membre en BDD
            $mdp = password_hash($_POST['mdp'], PASSWORD_DEFAULT); // Cette fonction prédéfinie permet de hasher le mot de passe selon l'algotithme actuel "bcrypt".
            // Il faudra lors de la connexion de comparer le hash de la BDD avec celui du mdp de l'internaute.
            // debug($mdp);
            ...
            if($succes){
                ...
                $contenu .= '<div class="alert alert-danger">Erreur lors de l\'enregistrement. Veuillez réessayer ultérieurment.</div>';
            }
        }
...
<form method="post" action="">
    ...
    <div>
        <div><label for="mdp">Password</label></div>
    ...
    <div>
        <div><input type="radio" name="civilite" value="m" checked>Homme
        <input type="radio" name="civilite" value="f"<?php if(isset($_POST['civilite'])&& $_POST['civilite']=='f' )echo 'checked';?>>Femme</div>
    </div>
    <div>
